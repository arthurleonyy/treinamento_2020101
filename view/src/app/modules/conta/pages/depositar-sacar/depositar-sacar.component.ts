...
export class DepositarSacarComponent extends FormBase implements OnInit {
  ...
  private depositar(conta: ContaDTO) {
    this.contaService.depositar(conta).subscribe(
      response => {
        SweetalertCustom.showAlertTimer('Deposito realizado com sucesso.', {type: 'success'}).then(
          result => {
            if (result.dismiss) {
  ...
  private sacar(conta: ContaDTO) {
    this.contaService.sacar(conta).subscribe(
      response => {
        SweetalertCustom.showAlertTimer('Saque realizado com sucesso.', {type: 'success'}).then(
          result => {
            if (result.dismiss) {
              this.router.navigate(['conta/operacoes']);
            }
